...
f = open('Prefetch_info.csv', 'w', encoding='utf-8')
wr = csv.writer(f)
...
for PF_file in PF_file_list:
	...
	try:
		...
		print('[**]File_Size:' + str(PF_file_Size[cut]))
...
for PF in PF_file_Name_List:
	if PF.endswith('.pf'):
		wr.writerow([PF, PF_file_Name[cut], PF_file_Size[cut], PF_file_Last_Run_Time[cut], PF_file_Created_Time[cut], PF_file_Modified_Time[cut], PF_file_Run_Count[cut]])
		cut+=1
print('Finished')